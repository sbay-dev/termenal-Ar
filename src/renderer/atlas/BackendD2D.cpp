...
void BackendD2D::_drawText(RenderingPayload& p)
{
    ...
    for (const auto row : p.rows)
    {
        ...
        for (const auto& m : row->mappings)
        {
            if (!m.fontFace)
            {
                baselineX = _drawBuiltinGlyphs(p, row, m, baselineY, baselineX, rtlShiftApplied);
                continue;
            }
            ...
            while (it != end)
            {
                ...
                while (++it != end && *it == fg && (!row->rtl || static_cast<size_t>(it - colorsBegin) != row->rtlGlyphStart))
                {
                }

                const auto count = it - beg;
                if (row->rtl && !rtlShiftApplied && static_cast<size_t>(off) >= row->rtlGlyphStart)
                {
                    baselineX += row->rtlPixelShift;
                    rtlShiftApplied = true;
                }
...
f32 BackendD2D::_drawBuiltinGlyphs(const RenderingPayload& p, const ShapedRow* row, const FontMapping& m, f32 baselineY, f32 baselineX, bool& rtlShiftApplied)
{
    ...
    {
        if (row->rtl && !rtlShiftApplied && i >= row->rtlGlyphStart)
        {
            baselineX += row->rtlPixelShift;
            rtlShiftApplied = true;
        }
...
void BackendD2D::_drawGridlineRow(const RenderingPayload& p, const ShapedRow* row, u16 y)
{
    ...
    const auto visualRange = [&](GridLineRange r) noexcept {
        if (row->rtl)
        {
            const auto width = p.s->viewportCellCount.x;
            const auto from = std::min<u16>(r.from, width);
            const auto to = std::min<u16>(r.to, width);
            r.from = width - to;
            r.to = width - from;
        }
        return r;
    ...
    for (const auto& logicalRange : row->gridLineRanges)
    {
        const auto r = visualRange(logicalRange);
