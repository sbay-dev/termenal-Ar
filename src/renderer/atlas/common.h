...
#pragma warning(pop)
    ...
    // RTL-VT / Arabic support (phase 1 plumbing).
    // Resolved bidi levels are stored independently of script-analysis runs because
    // their range boundaries do not always align (UAX#9 + script transitions partition
    // a line differently). Consumers that need shaping-ready segments must intersect
    // both vectors.
    struct BidiAnalysisSinkResult
    {
        uint32_t textPosition;
        uint32_t textLength;
        uint8_t resolvedLevel; // 0 = LTR; (level & 1) == 1 means RTL embedding.
    };
    ...
    struct ShapedRow
    {
        ...
        // True when the row's first strong character is RTL. Used by the private
        // RTL-VT build to mirror caret placement for Arabic console lines.
        bool rtl = false;
        // First physical column for cursor mirroring. Most RTL rows mirror from
        // column 0; fixed UI gutter markers like Copilot's colored vertical bar
        // can advance this so the gutter stays anchored while text is mirrored.
        u16 rtlContentStart = 0;
        // One past the last non-padding logical column in an RTL row. Cursor
        // mirroring clamps to this value so trailing prompt padding doesn't
        // create a visible gap between the caret and Arabic text.
        u16 rtlContentEnd = 0;
        // First glyph of the RTL visual block, plus the pixel shift that right
        // aligns the text while leaving any fixed UI gutter glyphs in place.
        u32 rtlGlyphStart = 0;
        f32 rtlPixelShift = 0;
