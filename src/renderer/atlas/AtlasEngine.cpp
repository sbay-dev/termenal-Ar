...
try
{
    ...
    if (const auto r = _api.invalidatedCursorArea; r.non_empty())
    {
        ...
        const auto rowsEnd = std::min<til::CoordType>(r.bottom, _p.s->viewportCellCount.y);
        for (auto y = r.top; y < rowsEnd; ++y)
        {
            ...
            const auto viewportWidth = static_cast<til::CoordType>(_p.s->viewportCellCount.x >> shift);
            const auto rtlContentStart = std::min<til::CoordType>(row->rtlContentStart, viewportWidth);
            const auto logicalLeft = std::clamp<til::CoordType>(r.left >> shift, rtlContentStart, viewportWidth);
            const auto logicalRight = std::clamp<til::CoordType>(r.right >> shift, rtlContentStart, viewportWidth);
            const auto visualLeft = (rtlContentStart + viewportWidth - logicalRight) << shift;
            const auto visualRight = (rtlContentStart + viewportWidth - logicalLeft) << shift;

            _p.dirtyRectInPx.left = std::min(_p.dirtyRectInPx.left, visualLeft * _p.s->font->cellSize.x);
            _p.dirtyRectInPx.top = std::min(_p.dirtyRectInPx.top, y * _p.s->font->cellSize.y);
            _p.dirtyRectInPx.right = std::max(_p.dirtyRectInPx.right, visualRight * _p.s->font->cellSize.x);
            _p.dirtyRectInPx.bottom = std::max(_p.dirtyRectInPx.bottom, (y + 1) * _p.s->font->cellSize.y);
        }
    }
...
try
{
    ...
    if (options.isOn)
    {
        ...
        if (_p.rows[top]->rtl)
        {
            // TODO(rtl-vt): Revisit double-width RTL rows if they become relevant for Arabic input.
            const auto viewportWidth = static_cast<til::CoordType>(_p.s->viewportCellCount.x >> shift);
            const auto rtlContentStart = std::min<til::CoordType>(_p.rows[top]->rtlContentStart, viewportWidth);
            const auto rtlContentEnd = std::clamp<til::CoordType>(_p.rows[top]->rtlContentEnd ? _p.rows[top]->rtlContentEnd : viewportWidth, rtlContentStart, viewportWidth);
            const auto logicalLeft = std::clamp<til::CoordType>(left, rtlContentStart, rtlContentEnd);
            if (options.cursorType == CursorType::VerticalBar)
            {
                left = rtlContentStart + viewportWidth - logicalLeft;
                right = left + cursorWidth;
            }
            else
            {
                const auto logicalRight = std::clamp<til::CoordType>(right, rtlContentStart, rtlContentEnd);
                const auto mirroredLeft = rtlContentStart + viewportWidth - logicalRight;
                const auto mirroredRight = rtlContentStart + viewportWidth - logicalLeft;
                left = mirroredLeft;
                right = mirroredRight;
            }
        }
...
void AtlasEngine::_flushBufferLine()
{
    ...
    if (row.rtl)
    {
        const auto rtlGlyphStart = std::min<size_t>(row.rtlGlyphStart, row.glyphAdvances.size());
        f32 fixedPrefixWidth = 0;
        for (size_t i = 0; i < rtlGlyphStart; ++i)
        {
            fixedPrefixWidth += row.glyphAdvances[i];
        }

        f32 rowWidth = 0;
        for (auto i = rtlGlyphStart; i < row.glyphAdvances.size(); ++i)
        {
            rowWidth += row.glyphAdvances[i];
        }

        const auto renditionShift = gsl::narrow_cast<u8>(row.lineRendition != LineRendition::SingleWidth);
        const auto viewportWidth = static_cast<f32>(_p.s->viewportCellCount.x >> renditionShift);
        row.rtlPixelShift = std::max(0.0f, viewportWidth * _p.s->font->cellSize.x - fixedPrefixWidth - rowWidth);

        const auto y = _api.lastPaintBufferLineCoord.y;
        const auto width = static_cast<size_t>(_p.s->viewportCellCount.x);
        const auto mirrorColorRow = [&](std::span<u32> bitmap) {
            auto beg = bitmap.begin() + _p.colorBitmapRowStride * y;
            std::reverse(beg, beg + width);
        };
        mirrorColorRow(_p.backgroundBitmap);
        mirrorColorRow(_p.foregroundBitmap);
        mirrorColorRow(_p.underlineBitmap);
    }
}
...
void AtlasEngine::_mapRegularText(size_t offBeg, size_t offEnd)
{
    ...
    const auto mapForward = [&](const size_t spanBeg, const size_t spanEnd) {
        for (u32 idx = gsl::narrow_cast<u32>(spanBeg), mappedEnd = 0; idx < spanEnd; idx = mappedEnd)
        {
            u32 mappedLength = 0;
            wil::com_ptr<IDWriteFontFace2> mappedFontFace;
            _mapCharacters(_api.bufferLine.data() + idx, gsl::narrow_cast<u32>(spanEnd - idx), &mappedLength, mappedFontFace.addressof());
            ...
            if (HasAnyStrongRtl(_api.bufferLine.data() + idx, mappedLength))
            {
                // RTL-VT: Never let Arabic/Hebrew/etc. take the "simple glyphs"
                // shortcut. GetTextComplexity may classify some fallback-font runs
                // as simple, but that path only copies one glyph per codepoint and
                // cannot apply Arabic joining, ligatures, mark placement, or bidi
                // levels. Force strong-RTL text through the DWrite shaping path.
                _mapComplex(mappedFontFace.get(), idx, mappedLength, row);
            }
            else if (_p.s->font->fontFeatures.empty())
            {
    ...
    const auto isWhitespace = [](const wchar_t w) noexcept {
        return w == L' ' || w == L'\t' || w == L'\u200B';
    };
    const auto isStrongLtr = [](const wchar_t w) noexcept {
        // Digits are bidi-weak. Treating them as strong LTR makes Arabic
        // numbered lists render as LTR paragraphs even when text is Arabic.
        return (w >= L'A' && w <= L'Z') || (w >= L'a' && w <= L'z');
    };
    size_t tableSeparators = 0;
    bool hasTableRule = false;
    size_t strongRtl = 0;
    size_t strongLtr = 0;
    std::optional<bool> firstStrongRtl;
    const auto rowBeg = size_t{ 0 };
    const auto rowEnd = _api.bufferLine.size();
    for (size_t i = rowBeg; i < rowEnd; ++i)
    {
        const auto w = _api.bufferLine[i];
        tableSeparators += (w == L'|' || w == L'+' || w == L'\u2502' || w == L'\u2503' || w == L'\u2551') ? 1 : 0;
        hasTableRule = hasTableRule ||
                       w == L'\u2500' || w == L'\u2501' || w == L'\u2550' ||
                       w == L'\u252C' || w == L'\u2534' || w == L'\u251C' ||
                       w == L'\u2524' || w == L'\u253C' || w == L'\u256C' ||
                       w == L'-' || w == L'=';
        ...
        char32_t cp = w;
        if (til::is_leading_surrogate(w) && i + 1 < rowEnd && til::is_trailing_surrogate(_api.bufferLine[i + 1]))
        {
            cp = til::combine_surrogates(w, _api.bufferLine[i + 1]);
            ++i;
        }
    ...
    if (tableSeparators >= 2 || (hasTableRule && tableSeparators >= 1))
    {
        mapForward(offBeg, offEnd);
        return;
    }

    const bool paragraphIsRtl = strongRtl != strongLtr ? strongRtl > strongLtr : firstStrongRtl.value_or(false);
    ...
    size_t rtlContentBeg = offBeg;
    auto rtlContentEnd = offEnd;
    if (rtlContentEnd == rtlContentBeg)
    {
        mapForward(offBeg, offEnd);
        return;
    }

    row.rtlContentStart = 0;
    row.rtl = true;
    auto rtlVisibleContentEnd = rtlContentEnd;
    while (rtlVisibleContentEnd > rtlContentBeg && isWhitespace(_api.bufferLine[rtlVisibleContentEnd - 1]))
    {
        --rtlVisibleContentEnd;
    }
    const auto rtlContentEndColumn = rtlVisibleContentEnd < _api.bufferLineColumn.size() ? _api.bufferLineColumn[rtlVisibleContentEnd] : _api.bufferLineColumn.back();
    row.rtlContentEnd = gsl::narrow_cast<u16>(rtlContentEndColumn);

    mapForward(offBeg, rtlContentBeg);
    const auto rtlGlyphStart = row.glyphIndices.size();

    std::vector<std::pair<size_t, size_t>> tokens;
    for (size_t tokenBeg = rtlContentBeg; tokenBeg < rtlContentEnd;)
    {
        const bool whitespace = isWhitespace(_api.bufferLine[tokenBeg]);
        size_t tokenEnd = tokenBeg + 1;
        while (tokenEnd < rtlContentEnd && isWhitespace(_api.bufferLine[tokenEnd]) == whitespace)
        {
            ++tokenEnd;
        }
    ...
    {
        mapForward(it->first, it->second);
    }

    row.rtlGlyphStart = gsl::narrow_cast<u32>(rtlGlyphStart);
    f32 fixedPrefixWidth = 0;
    for (size_t i = 0; i < rtlGlyphStart; ++i)
    {
        fixedPrefixWidth += row.glyphAdvances[i];
    ...
    f32 rtlContentWidth = 0;
    for (auto i = rtlGlyphStart; i < row.glyphAdvances.size(); ++i)
    {
        rtlContentWidth += row.glyphAdvances[i];
    }

    const auto shift = gsl::narrow_cast<u8>(row.lineRendition != LineRendition::SingleWidth);
    const auto viewportWidth = static_cast<f32>(_p.s->viewportCellCount.x >> shift);
    row.rtlPixelShift = std::max(0.0f, viewportWidth * _p.s->font->cellSize.x - fixedPrefixWidth - rtlContentWidth);
}
...
void AtlasEngine::_mapComplex(IDWriteFontFace2* mappedFontFace, u32 idx, u32 length, ShapedRow& row)
{
    ...
    // RTL-VT phase 1A: pre-scan the run for any strong-RTL codepoint. This
    // single linear pass is the gate that keeps the LTR path bit-identical to
    // the pre-RTL behavior -- when no strong-RTL char is present we skip the
    // AnalyzeBidi call entirely and use the legacy single-sink constructor,
    // exactly as the renderer always has.
    const bool hasRtl = HasAnyStrongRtl(_api.bufferLine.data() + idx, length);

    TextAnalysisSource analysisSource{ _p.userLocaleName.c_str(), _api.bufferLine.data(), gsl::narrow<UINT32>(_api.bufferLine.size()) };

    if (hasRtl)
    {
        // Bidi-aware sink: receives both SetScriptAnalysis and SetBidiLevel
        // callbacks. The collected bidi runs are stored in _api.bidiResults
        // for future shaping-path consumption (phase 1B will intersect them
        // with the script runs to decide isRightToLeft per shaping segment).
    ...
    {
        // LTR fast path: legacy single-sink, no bidi callbacks. Bit-identical
        // to the pre-RTL behavior for every ASCII / Latin / CJK / neutral line.
    ...
    for (const auto& a : _api.analysisResults)
    {
        ...
        // RTL-VT phase 1B: determine resolved bidi direction for this script run.
        // If DWrite's bidi run boundaries don't align exactly with the script
        // run, fall back to the script run's own strong-RTL content. This keeps
        // Arabic/Hebrew shaping enabled even for short fallback-font runs.
        const bool runHasStrongRtl = HasAnyStrongRtl(_api.bufferLine.data() + a.textPosition, a.textLength);
        bool runIsRtl = runHasStrongRtl;
        for (const auto& b : _api.bidiResults)
        {
            const u32 bEnd = b.textPosition + b.textLength;
            if (b.textPosition <= a.textPosition && bEnd > a.textPosition)
            {
                runIsRtl = (b.resolvedLevel & 1u) == 1u;
                break;
            }
        }
...
#pragma warning(suppress : 26492) // Don't use const_cast to cast away const or volatile (type.3).
        ...
        // RTL-VT phase 1B: walk clusters in logical order for accounting, then
        // append RTL clusters in visual order for Atlas' left-to-right painter.
        //
        // DWrite shaping must receive logical text and runIsRtl=true to choose
        // Arabic joining forms. Atlas, however, paints glyph arrays from left to
        // right. For an Arabic run beginning at column 0, the visually-leftmost
        // cluster is the last logical cluster, so we reverse cluster order at the
        // row append step. We do NOT reverse glyphs inside a cluster; ligatures
        // and marks must keep DWrite's internal cluster order.
        struct RtlVtCluster
        {
            u32 glyphLo;
            u32 glyphHi;
        ...
        std::vector<RtlVtCluster> rtlClusters;
        if (runIsRtl)
        {
            rtlClusters.reserve(a.textLength);
        }

        auto prevCluster = _api.clusterMap[0];
        if (prevCluster > actualGlyphCount)
        {
            THROW_HR_MSG(E_UNEXPECTED, "rtl-vt: clusterMap start escapes glyph array (start=%u count=%u rtl=%d)",
                         static_cast<u32>(prevCluster), actualGlyphCount, runIsRtl ? 1 : 0);
        }
        size_t beg = 0;
        ...
        for (size_t i = 1; i <= a.textLength; ++i)
        {
            ...
            // Defensive: if the cluster map ever violates the non-decreasing
            // invariant or escapes the glyph array, bail out cleanly rather
            // than risk OOB reads or a ~65K-element insert blow-up.
            if (nextCluster < prevCluster || nextCluster > actualGlyphCount)
            {
                THROW_HR_MSG(E_UNEXPECTED, "rtl-vt: unexpected clusterMap topology (prev=%u next=%u count=%u rtl=%d)",
                             static_cast<u32>(prevCluster), static_cast<u32>(nextCluster), actualGlyphCount, runIsRtl ? 1 : 0);
            }
            ...
            if (runIsRtl)
            {
                rtlClusters.push_back({
                    static_cast<u32>(prevCluster),
                    static_cast<u32>(nextCluster),
                    fg,
                });
            }
            else
        ...
        if (runIsRtl)
        {
            for (auto it = rtlClusters.rbegin(); it != rtlClusters.rend(); ++it)
            {
                row.glyphIndices.insert(row.glyphIndices.end(), _api.glyphIndices.begin() + it->glyphLo, _api.glyphIndices.begin() + it->glyphHi);
                row.glyphAdvances.insert(row.glyphAdvances.end(), _api.glyphAdvances.begin() + it->glyphLo, _api.glyphAdvances.begin() + it->glyphHi);
                row.glyphOffsets.insert(row.glyphOffsets.end(), _api.glyphOffsets.begin() + it->glyphLo, _api.glyphOffsets.begin() + it->glyphHi);
                row.colors.insert(row.colors.end(), it->glyphHi - it->glyphLo, it->color);
            }
        }
        else
        {
