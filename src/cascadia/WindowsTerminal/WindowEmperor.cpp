...
void WindowEmperor::_setupSessionPersistence(bool enabled)
{
    ...
    _persistStateTimer.Interval(std::chrono::minutes(1));
    _persistStateTimer.Tick([&](auto&&, auto&&) {
        try
        {
            _persistState(ApplicationState::SharedInstance(), true);
        }
        CATCH_LOG();
    });
...
void WindowEmperor::_persistState(const ApplicationState& state, const bool persistContent) const
{
    ...
    if (persistContent)
    {
        _persistTerminalBuffers(false);
    }
}

void WindowEmperor::_persistTerminalBuffers(const bool cleanupStaleFiles) const
{
    ...
    if (!persistBuffers && !cleanupStaleFiles)
    {
        return;
    }
...
void WindowEmperor::_finalizeSessionPersistence() const
{
    ...
    _persistTerminalBuffers(_needsPersistenceCleanup);
}
