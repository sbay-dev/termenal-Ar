...
#pragma warning(disable : 26490) // Don't use reinterpret_cast (type.1).
    ...
    winrt::hstring ConptyConnection::CopilotAgentSessionId() const
    {
        try
        {
            ...
            const auto rootPid = GetProcessId(_piClient.hProcess);
            if (rootPid == 0)
            {
                return {};
            }

            for (const auto pid : _descendantProcessIds(rootPid))
            {
                wil::unique_handle process{ OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION | PROCESS_VM_READ, FALSE, pid) };
                if (!process)
                {
                    continue;
                }
                ...
                try
                {
                    if (auto sessionId = _environmentValueFromProcess(process.get(), L"COPILOT_AGENT_SESSION_ID"))
                    {
                        return winrt::hstring{ *sessionId };
                    }
                    const auto commandline = _commandlineFromProcess(process.get());
                    if (auto sessionId = _copilotSessionIdFromCommandline({ commandline.c_str(), commandline.size() }))
                    {
                        return winrt::hstring{ *sessionId };
                    }
                }
    ...
    std::optional<std::wstring> ConptyConnection::_copilotSessionIdFromCommandline(std::wstring_view commandline)
    {
        const auto isGuidChar = [](wchar_t ch) noexcept {
            return (ch >= L'0' && ch <= L'9') ||
                   (ch >= L'a' && ch <= L'f') ||
                   (ch >= L'A' && ch <= L'F') ||
                   ch == L'-';
        };
        const auto extractAfter = [&](std::wstring_view marker) -> std::optional<std::wstring> {
            const auto pos = commandline.find(marker);
            if (pos == std::wstring_view::npos)
            {
                return std::nullopt;
            }

            auto valueBeg = pos + marker.size();
            while (valueBeg < commandline.size() && commandline[valueBeg] == L' ')
            {
                ++valueBeg;
            }

            auto valueEnd = valueBeg;
            while (valueEnd < commandline.size() && isGuidChar(commandline[valueEnd]))
            {
                ++valueEnd;
            }

            if (valueEnd - valueBeg == 36)
            {
                return std::wstring{ commandline.substr(valueBeg, valueEnd - valueBeg) };
            }

            return std::nullopt;
        };

        if (auto id = extractAfter(L"--resume="))
        {
            return id;
        }
        if (auto id = extractAfter(L"--resume "))
        ...
        if (auto id = extractAfter(L"--session-id="))
        {
            return id;
        }
        if (auto id = extractAfter(L"--session-id "))
    ...
    std::optional<std::wstring> ConptyConnection::_environmentValueFromProcess(HANDLE process, std::wstring_view name)
    {
        struct PROCESS_BASIC_INFORMATION
        {
            NTSTATUS ExitStatus;
        ...
        struct RTL_USER_PROCESS_PARAMETERS_WITH_ENVIRONMENT
        {
            BYTE Reserved1[16];
            PVOID Reserved2[10];
        ...
        if (VirtualQueryEx(process, params.Environment, &memoryInfo, sizeof(memoryInfo)) == 0)
        {
            return std::nullopt;
        }

        const auto environmentAddress = reinterpret_cast<uintptr_t>(params.Environment);
        const auto regionBase = reinterpret_cast<uintptr_t>(memoryInfo.BaseAddress);
        if (environmentAddress < regionBase)
        {
            return std::nullopt;
        }

        const auto offsetIntoRegion = environmentAddress - regionBase;
        if (offsetIntoRegion >= memoryInfo.RegionSize)
        {
            return std::nullopt;
        }

        constexpr size_t maxEnvironmentBytes = 1024 * 1024;
        const auto availableBytes = memoryInfo.RegionSize - offsetIntoRegion;
        const auto bytesToRead = std::min<size_t>(availableBytes, maxEnvironmentBytes);
        std::vector<wchar_t> environment((bytesToRead / sizeof(wchar_t)) + 1);
        SIZE_T bytesRead{};
        THROW_IF_WIN32_BOOL_FALSE(ReadProcessMemory(process, params.Environment, environment.data(), bytesToRead, &bytesRead));
        environment[bytesRead / sizeof(wchar_t)] = L'\0';

        const auto* cursor = environment.data();
        const auto* end = cursor + (bytesRead / sizeof(wchar_t));
        while (cursor < end && *cursor != L'\0')
        {
            const std::wstring_view entry{ cursor };
            if (const auto equals = entry.find(L'='); equals != std::wstring_view::npos)
            {
                const auto key = entry.substr(0, equals);
                if (til::equals_insensitive_ascii(key, name))
                {
                    return std::wstring{ entry.substr(equals + 1) };
                }
            }
            cursor += entry.size() + 1;
        }
    ...
    std::vector<DWORD> ConptyConnection::_descendantProcessIds(DWORD rootProcessId)
    {
        ...
        const auto closeSnapshot = wil::scope_exit([&] {
        ...
        {
            const auto parentPid = result[i];
            for (const auto& process : processes)
            {
                if (process.th32ParentProcessID == parentPid &&
                    std::find(result.begin(), result.end(), process.th32ProcessID) == result.end())
                {
