...
{
    static bool _containsInsensitiveAscii(std::wstring_view text, std::wstring_view needle)
    {
        if (needle.empty())
        {
            return true;
        ...
        {
            if (til::equals_insensitive_ascii(text.substr(i, needle.size()), needle))
            {
                return true;
            }
        }
    ...
    static winrt::hstring _buildCopilotResumeCommandline(const winrt::hstring& sessionId, const winrt::hstring& rootCommandline)
    {
        const std::wstring root{ rootCommandline };
        const auto shell = _containsInsensitiveAscii(root, L"pwsh") ? std::wstring_view{ L"pwsh.exe" } : std::wstring_view{ L"powershell.exe" };
        const std::wstring resumeCommand = L"copilot --resume=" + std::wstring{ sessionId };

        std::wstring commandline;
        commandline.reserve(shell.size() + resumeCommand.size() + 64);
        commandline.append(shell);
        commandline.append(L" -NoLogo -NoExit -ExecutionPolicy Bypass -Command ");
        commandline.append(resumeCommand);
    ...
    INewContentArgs TerminalPaneContent::GetNewTerminalArgs(const BuildStartupKind kind) const
    {
        ...
        case BuildStartupKind::Persist:
        {
            ...
            if (connection)
            {
                ...
                {
                    if (const auto copilotSessionId = conpty.CopilotAgentSessionId(); !copilotSessionId.empty())
                    {
                        args.Commandline(_buildCopilotResumeCommandline(copilotSessionId, conpty.Commandline()));
                        if (args.TabTitle().empty())
                        {
                            args.TabTitle(L"GitHub Copilot");
                        }
                        break;
                    }
                }
