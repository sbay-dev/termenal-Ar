...
#undef ON_ALL_ACTIONS
    ...
    void TerminalPage::PersistState()
    {
        ...
        std::optional<uint32_t> persistedFocusedIndex;
        uint32_t persistedTabCount = 0;
        ...
        for (uint32_t i = 0; i < tabCount; ++i)
        {
            ...
            if (!t->IsContinuityPinned())
            {
                continue;
            }

            if (focusedTabIndex && *focusedTabIndex == i)
            {
                persistedFocusedIndex = persistedTabCount;
            }
            ++persistedTabCount;
        ...
        if (persistedFocusedIndex && *persistedFocusedIndex != persistedTabCount - 1)
        {
            ActionAndArgs action;
            action.Action(ShortcutAction::SwitchToTab);
            SwitchToTabArgs switchToTabArgs{ persistedFocusedIndex.value() };
