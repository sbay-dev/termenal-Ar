...
namespace winrt::TerminalApp::implementation
{
    ...
    std::vector<ActionAndArgs> Tab::BuildStartupActions(BuildStartupKind kind) const
    {
...

        {
            ...
            if (wasSettings)
            {
                ...
                std::vector<ActionAndArgs> settingsActions{ std::move(newTabAction) };
                if (kind == BuildStartupKind::Persist && _pinnedForContinuity)
                {
                    ActionAndArgs pinTabAction{};
                    pinTabAction.Action(ShortcutAction::ToggleTabContinuityPin);
                    settingsActions.emplace_back(std::move(pinTabAction));
                }
        ...
        if (kind == BuildStartupKind::Persist && _pinnedForContinuity)
        {
            ActionAndArgs pinTabAction{};
            pinTabAction.Action(ShortcutAction::ToggleTabContinuityPin);
            state.args.emplace_back(std::move(pinTabAction));
        }
    ...
    void Tab::SetContinuityPinned(const bool pinned)
    {
        ASSERT_UI_THREAD();

        if (_pinnedForContinuity != pinned)
        {
            _pinnedForContinuity = pinned;
            _UpdateContinuityPinMenuItem();
        }
    }

    void Tab::ToggleContinuityPinned()
    {
        ASSERT_UI_THREAD();
        SetContinuityPinned(!_pinnedForContinuity);
    }

    bool Tab::IsContinuityPinned() const noexcept
    {
        return _pinnedForContinuity;
    }
    ...
    void Tab::_CreateContextMenu()
    {
        ...
        {
            Controls::FontIcon continuityPinSymbol;
            continuityPinSymbol.FontFamily(Media::FontFamily{ L"Segoe Fluent Icons, Segoe MDL2 Assets" });
            continuityPinSymbol.Glyph(L"\xE718"); // Pin

            _continuityPinMenuItem.Click([weakThis](auto&&, auto&&) {
                if (auto tab{ weakThis.get() })
                {
                    ActionAndArgs togglePinAction{};
                    togglePinAction.Action(ShortcutAction::ToggleTabContinuityPin);
                    tab->_dispatch.DoAction(*tab, togglePinAction);
                }
            });
            _continuityPinMenuItem.Icon(continuityPinSymbol);
            _UpdateContinuityPinMenuItem();
        }
        ...
        contextMenuFlyout.Items().Append(_continuityPinMenuItem);
    ...
    void Tab::_UpdateContinuityPinMenuItem()
    {
        if (!_continuityPinMenuItem)
        {
            return;
        }

        const auto text = _pinnedForContinuity ? RS_(L"ContinuityUnpinTabText") : RS_(L"ContinuityPinTabText");
        const auto toolTip = _pinnedForContinuity ? RS_(L"ContinuityUnpinTabToolTip") : RS_(L"ContinuityPinTabToolTip");
        _continuityPinMenuItem.Text(text);
        WUX::Controls::ToolTipService::SetToolTip(_continuityPinMenuItem, box_value(toolTip));
        Automation::AutomationProperties::SetHelpText(_continuityPinMenuItem, toolTip);
    }
