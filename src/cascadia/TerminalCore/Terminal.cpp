...
namespace
{
    ...
    constexpr bool isRtlHighSurrogate(const wchar_t ch) noexcept
    {
        return ch >= 0xD800 && ch <= 0xDBFF;
    }

    constexpr bool isRtlLowSurrogate(const wchar_t ch) noexcept
    {
        return ch >= 0xDC00 && ch <= 0xDFFF;
    }

    bool isStrongRtlChar(const char32_t cp) noexcept
    {
        for (const auto& range : g_rtlRanges)
        {
    ...
    bool isStrongLtrChar(const wchar_t ch) noexcept
    {
        return (ch >= L'A' && ch <= L'Z') || (ch >= L'a' && ch <= L'z');
    }

    bool isMajorityStrongRtlText(const std::wstring_view text) noexcept
    {
        size_t strongRtl = 0;
        size_t strongLtr = 0;
        ...
        for (size_t i = 0; i < text.size(); ++i)
        {
            ...
            char32_t cp = ch;
            if (isRtlHighSurrogate(ch) && i + 1 < text.size() && isRtlLowSurrogate(text[i + 1]))
            {
                cp = 0x10000u + ((static_cast<char32_t>(ch) - 0xD800u) << 10) +
...
TerminalInput::OutputType Terminal::SendKeyEvent(const WORD vkey,
                                                 ...
                                                 const bool keyDown)
{
    auto effectiveVkey = vkey;
    bool mirroredRtlArrowKey = false;
    if ((effectiveVkey == VK_LEFT || effectiveVkey == VK_RIGHT) && keyDown)
    {
        const auto cursor = _activeBuffer().GetCursor().GetPosition();
        const auto& row = _activeBuffer().GetRowByOffset(cursor.y);
        if (isMajorityStrongRtlText(row.GetText()))
        {
            effectiveVkey = effectiveVkey == VK_LEFT ? VK_RIGHT : VK_LEFT;
            mirroredRtlArrowKey = true;
        }
    }
    ...
    if (IsInputKey(effectiveVkey) && keyDown)
    {
        TrySnapOnInput();
    }

    _StoreKeyEvent(effectiveVkey, scanCode);
    ...
    // GH#7064
    if (effectiveVkey == 0 || effectiveVkey >= 0xff)
    {
        return {};
    }
    ...
    // GH#7495
    const auto sc = scanCode && !mirroredRtlArrowKey ? scanCode : _ScanCodeFromVirtualKey(effectiveVkey);
    if (sc == 0)
    {
        return {};
    }
    ...
    const auto ch = isSuppressedAltGrAlias ? UNICODE_NULL : _CharacterFromKeyEvent(effectiveVkey, sc, states);
    ...
    if (keyDown && !isAltOnlyPressed && effectiveVkey != VK_TAB && ch != UNICODE_NULL)
    {
        return {};
    }

    const auto keyEv = SynthesizeKeyEvent(keyDown, 1, effectiveVkey, sc, ch, states.Value());
