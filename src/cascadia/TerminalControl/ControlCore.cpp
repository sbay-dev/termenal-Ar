...
namespace winrt::Microsoft::Terminal::Control::implementation
{
    ...
    static bool rtlvtIsStrongRtl(const wchar_t w) noexcept
    {
        // Phase 1 intentionally targets BMP RTL scripts used by Arabic/Hebrew terminals.
        return (w >= 0x0590 && w <= 0x08FF) ||
               (w >= 0xFB1D && w <= 0xFDFF) ||
               (w >= 0xFE70 && w <= 0xFEFF);
    }

    static bool rtlvtIsStrongLtr(const wchar_t w) noexcept
    {
        return (w >= L'A' && w <= L'Z') || (w >= L'a' && w <= L'z');
    }

    static bool rtlvtGlyphIsWhitespace(const std::wstring_view glyph) noexcept
    {
        return glyph.size() == 1 && (glyph.front() == L' ' || glyph.front() == L'\t');
    }

    static bool rtlvtGlyphIsTableSeparator(const std::wstring_view glyph) noexcept
    {
        if (glyph.size() != 1)
        {
            return false;
    ...
    static bool rtlvtGlyphIsLeftGutter(const std::wstring_view glyph) noexcept
    {
        if (glyph.size() != 1)
        {
            return false;
    ...
    static bool rtlvtRowIsRtl(const ROW& row) noexcept
    {
        const auto width = gsl::narrow_cast<til::CoordType>(row.size());
        size_t strongRtl = 0;
        size_t strongLtr = 0;
        size_t tableSeparators = 0;
        std::optional<bool> firstStrongRtl;

        for (til::CoordType col = 0; col < width; ++col)
        {
            const auto glyph = row.GlyphAt(col);
            tableSeparators += rtlvtGlyphIsTableSeparator(glyph) ? 1 : 0;
            for (const auto w : glyph)
            {
                if (rtlvtIsStrongRtl(w))
                {
                    ++strongRtl;
                    if (!firstStrongRtl)
                    {
        ...
        if (tableSeparators >= 2)
        {
            return false;
        }

        return strongRtl != strongLtr ? strongRtl > strongLtr : firstStrongRtl.value_or(false);
    }

    static til::CoordType rtlvtContentStartColumn(const ROW& row, const til::CoordType width) noexcept
    {
        static constexpr til::CoordType maxGutterColumn = 3;
        auto col = til::CoordType{ 0 };
        while (col < width && rtlvtGlyphIsWhitespace(row.GlyphAt(col)))
        {
            ++col;
        }
        ...
        for (;;)
        {
            if (col >= width || col > maxGutterColumn || !rtlvtGlyphIsLeftGutter(row.GlyphAt(col)))
            {
                return col;
            }
    ...
    static til::CoordType rtlvtVisualToLogicalColumn(const ROW& row, til::CoordType visualColumn)
    {
        // TODO(rtl-vt): Revisit double-width RTL rows if Windows Terminal exposes them for Arabic input.
        // This mouse-reposition path assumes the row width matches the viewport width used by Atlas cursor mirroring.
        const auto width = gsl::narrow_cast<til::CoordType>(row.size());
        visualColumn = std::clamp(visualColumn, til::CoordType{ 0 }, width);

        if (width == 0 || !rtlvtRowIsRtl(row))
        {
            return visualColumn;
        }

        const auto logicalEnd = std::clamp(row.MeasureRight(), til::CoordType{ 0 }, width);
        const auto visualStart = width - logicalEnd;
        if (visualColumn <= visualStart)
        {
            return logicalEnd;
        }
        if (visualColumn >= width)
        {
            return 0;
        }
        ...
        for (til::CoordType beg = 0; beg < logicalEnd;)
        {
            const bool whitespace = rtlvtGlyphIsWhitespace(row.GlyphAt(beg));
            auto end = beg + 1;
            while (end < logicalEnd && rtlvtGlyphIsWhitespace(row.GlyphAt(end)) == whitespace)
            {
                ++end;
            }
            spans.push_back({ beg, end, whitespace });
            beg = end;
        }

        auto visualOffset = visualColumn - visualStart;
        for (auto it = spans.rbegin(); it != spans.rend(); ++it)
        {
            const auto spanWidth = it->end - it->beg;
            if (visualOffset <= spanWidth)
            {
                return it->beg + visualOffset;
            }
            visualOffset -= spanWidth;
        }

        return 0;
    }

    static til::point rtlvtVisualToLogicalPoint(::Microsoft::Terminal::Core::Terminal& terminal, til::point terminalPosition)
    {
        const auto viewport = terminal.GetViewport();
        terminalPosition.x = std::clamp(terminalPosition.x, 0, viewport.Width());
        terminalPosition.y = std::clamp(terminalPosition.y, 0, viewport.Height() - 1);

        const auto rawBufferPos = viewport.Origin() + terminalPosition;
        const auto& buffer = terminal.GetTextBuffer();
        if (rawBufferPos.y >= 0 && rawBufferPos.y < buffer.TotalRowCount())
        {
            const auto& row = buffer.GetRowByOffset(rawBufferPos.y);
            terminalPosition.x = rtlvtVisualToLogicalColumn(row, terminalPosition.x);
        }
    ...
    void ControlCore::_updateHoveredCell(const std::optional<til::point> terminalPosition)
    {
        ...
        {
            const auto lock = _terminal->LockForReading();
            const auto hitPosition = rtlvtVisualToLogicalPoint(*_terminal, *terminalPosition);
            newId = _terminal->GetHyperlinkIdAtViewportPosition(hitPosition);
            newInterval = _terminal->GetHyperlinkIntervalFromViewportPosition(hitPosition);
        }
    ...
    {
        const auto lock = _terminal->LockForReading();
        const auto visualPos = til::point{ pos };
        const auto logicalPos = rtlvtVisualToLogicalPoint(*_terminal, visualPos);
        if (auto uri = _terminal->GetHyperlinkAtViewportPosition(logicalPos); !uri.empty())
        {
            return winrt::hstring{ uri };
        }

        const auto viewport = _terminal->GetViewport();
        const auto rawBufferPos = viewport.Origin() + visualPos;
        const auto& buffer = _terminal->GetTextBuffer();
        if (rawBufferPos.y >= 0 && rawBufferPos.y < buffer.TotalRowCount())
        {
            const auto& row = buffer.GetRowByOffset(rawBufferPos.y);
            if (rtlvtRowIsRtl(row))
            {
                const auto rowRight = std::clamp(row.MeasureRight(), til::CoordType{ 0 }, gsl::narrow_cast<til::CoordType>(row.size()));
                for (til::CoordType x = 0; x < rowRight; ++x)
                {
                    if (auto uri = _terminal->GetHyperlinkAtBufferPosition({ x, rawBufferPos.y }); !uri.empty())
                    {
                        return winrt::hstring{ uri };
                    }
                }
    ...
    winrt::hstring ControlCore::HoveredUriText() const
    {
        ...
        {
            const auto lock = _terminal->LockForReading();
            auto uri{ _terminal->GetHyperlinkAtViewportPosition(rtlvtVisualToLogicalPoint(*_terminal, *_lastHoveredCell)) };
    ...
    {
        const auto lock = _terminal->LockForWriting();
        _terminal->SetSelectionAnchor(rtlvtVisualToLogicalPoint(*_terminal, position));
    }
    ...
    void ControlCore::SetEndSelectionPoint(const til::point position)
    {
        ...
        auto terminalPosition = rtlvtVisualToLogicalPoint(*_terminal, position);
                                          ...
                                          bool& selectionNeedsToBeCopied)
    {
        ...
        const auto selectionTerminalPosition = rtlvtVisualToLogicalPoint(*_terminal, terminalPosition);
        ...
        if (shiftEnabled && _terminal->IsSelectionActive())
        {
            ...
            _terminal->SetSelectionEnd(selectionTerminalPosition, mode);
        ...
        else if (mode != ::Terminal::SelectionExpansion::Char || shiftEnabled)
        {
            ...
            _terminal->MultiClickSelection(selectionTerminalPosition, mode);
            selectionNeedsToBeCopied = true;
        }
    ...
    void ControlCore::_repositionCursorWithMouse(const til::point terminalPosition)
    {
        ...
        if (!marks.empty())
        {
            ...
            const auto viewportOrigin = _terminal->GetViewport().Origin();
            auto adjustedTerminalPosition = terminalPosition;
            const auto rawBufferPos = viewportOrigin + terminalPosition;
            if (rawBufferPos.y >= 0 && rawBufferPos.y < buffer.TotalRowCount())
            {
                const auto& row = buffer.GetRowByOffset(rawBufferPos.y);
                adjustedTerminalPosition.x = rtlvtVisualToLogicalColumn(row, terminalPosition.x);
            }

            const auto bufferPos = viewportOrigin + adjustedTerminalPosition;
