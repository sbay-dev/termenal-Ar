    ...
    _fCopyColor(false)
{
    ...
    wcscpy_s(_FaceName, L"Cascadia Mono");
...
{
    // RTL-VT private build: always use the DirectX/Atlas renderer in OpenConsole
    // so Arabic shaping changes in renderer/atlas are actually exercised. The
    // stock default is GDI, which cannot consume the new DWrite bidi shaping path.
    return true;
}
